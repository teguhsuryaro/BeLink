# 04 - Pencarian Mitra dan OSRM

## Tujuan
Implementasi logika pencarian mitra terdekat dan kalkulasi jarak rute menggunakan OSRM API.

---

## Langkah-Langkah

### 1. Buat Hook useOrders

**BUAT FILE**: `src/hooks/useOrders.ts`
...
- **Format parameter**: longitude dulu, baru latitude (kebalikan dari Leaflet!)
- **Gratis**: tidak perlu API key, tidak ada limit eksplisit (tapi jangan abuse)
- **Response**: jarak dalam meter (`route.distance`) dan durasi dalam detik (`route.duration`)
- **Fallback**: jika OSRM gagal (server down, dll), gunakan haversine (jarak garis lurus)

---

## Validasi

- [ ] File `src/hooks/useOrders.ts` sudah ada
- [ ] Fungsi `calculateRouteDistance` bisa dipanggil dan mengembalikan jarak
- [ ] Fungsi `searchNearbyMitras` bisa mengquery mitra dari database
- [ ] Hook `useOrders` mengembalikan `activeOrder`, `history`, `updateOrderStatus`, `cancelOrder`
- [ ] Jalankan `npm run dev` — tidak ada error TypeScript

---

**Selesai? Lanjut ke `05-halaman-active-order.md`**
